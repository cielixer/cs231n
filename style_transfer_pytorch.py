...
def content_loss(content_weight, content_current, content_original):
    ...
    M_current = torch.reshape(content_current, (C_l, -1))
    M_original = torch.reshape(content_original, (C_l, -1))

    return torch.sum(torch.pow(M_current - M_original, 2)) * content_weight
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for idx, l in zip(range(len(style_layers)), style_layers):
      A = gram_matrix(feats[l])
      G = style_targets[idx]
      E = torch.sum(torch.pow(G-A, 2))
      style_loss += E * style_weights[idx]
...
def tv_loss(img, tv_weight):
    ...
    loss = torch.sum(torch.pow(right, 2)) + torch.sum(torch.pow(down, 2))
    loss *= tv_weight
